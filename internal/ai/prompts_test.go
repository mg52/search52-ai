...
func validPromptFiles() map[string]string {
	return map[string]string{
		"tagging_system.template":     "tagging max {{.MaxTags}}",
		"tagging_user.template":       "existing {{.ExistingTags}} doc {{.Content}}",
		"description_system.template": "describe {{.TagName}}",
		"description_user.template":   "tag {{.TagName}} ex {{.Examples}}",
	}
}
...
func TestLoadPromptsBadTemplate(t *testing.T) {
	files := validPromptFiles()
	files["tagging_system.template"] = "broken {{.MaxTags" // unterminated action
	dir := writePromptDir(t, files)
	if _, err := LoadPrompts(dir); err == nil {
