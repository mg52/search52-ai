// Package embedding is a self-contained, in-memory vector search index. A
// single SearchEngine owns everything: the documents, the
// incrementally-discovered categories (centroids), the category→documents
// index, the clustering math, and the search itself. There is no LLM and no
// separate persistence layer — one struct, one mutex.
//
...
type Config struct {
	...
	// DisableSplit turns off the async splitCategory launch in Process.
	// ShouldSplit is still tracked while disabled, so re-enabling picks up
	// any category that already qualified.
	DisableSplit bool
}
...
func New(name string, embedder common.Embedder, cfg Config) *SearchEngine {
	cfg.withDefaults()
	return &SearchEngine{
		name:              name,
		embedder:          embedder,
		...
		splitDisabled:     cfg.DisableSplit,
...
func (se *SearchEngine) Kind() string { return "embedding" }
...
// state on its own.
func (se *SearchEngine) Process(ctx context.Context, id, content string) (common.Document, error) {
	if id == "" {
	...
	if err != nil {
		return common.Document{}, fmt.Errorf("embedding document: %w", err)
	}
	norm := vec.Norm(v)
	if norm == 0 {
		return common.Document{}, fmt.Errorf("document embedding is a zero vector")
	}
	...
	for _, a := range assignments {
		...
		if c, ok := se.categories[a.name]; ok && c.ShouldSplit && !se.splitting[a.name] && !se.splitDisabled {
...
func (se *SearchEngine) GetDocument(id string) (common.Document, bool) {
	se.mu.RLock()
	defer se.mu.RUnlock()
	d, ok := se.docs[id]
	if !ok {
...
// left empty).
func (se *SearchEngine) Remove(id string) (common.Document, bool) {
	se.mu.Lock()
	defer se.mu.Unlock()
	doc, ok := se.docs[id]
	if !ok {
...
func (se *SearchEngine) ListCategories() []common.CategorySummary {
	se.mu.RLock()
	defer se.mu.RUnlock()
	out := make([]common.CategorySummary, 0, len(se.categories))
	for _, c := range se.categories {
		out = append(out, c.summary(len(se.catDocs[c.Name])))
	}
	return out
}

// GetCategory returns the category named name, if present.
func (se *SearchEngine) GetCategory(name string) (common.CategorySummary, bool) {
	se.mu.RLock()
	defer se.mu.RUnlock()
	...
	return c.summary(len(se.catDocs[name])), true
}
...
func (se *SearchEngine) Search(ctx context.Context, query string, limit int) (common.Results, error) {
	if limit <= 0 {
		limit = 10
	}
	...
	// beyond topN.
	catCands := make([]common.CategoryMatch, 0, se.topN)
	ch := make([]common.InternalHit, 0, se.topN)
	...
	dh := make([]common.InternalHit, 0, limit)
	...
	for k, m := range matched {
		for id := range se.catDocs[m.Name] {
			...
			if len(dh) < limit {
				...
				dh = common.HeapPushHit(dh, common.InternalHit{ID: uint32(len(docCands) - 1), Score: score})
			} else if dh[0].Score < score {
				slot := dh[0].ID
				docCands[slot] = docCand{doc, sim}
				common.HeapReplaceTop(dh, common.InternalHit{ID: slot, Score: score})
			}
		}
	}

	// Drain the document heap into descending order.
	results := make([]common.SearchResult, len(dh))
	for i := len(dh) - 1; i >= 0; i-- {
		hit := dh[0]
		if i > 0 {
			dh[0] = dh[i]
			common.SiftDownHit(dh, 0, i)
		}
		dc := docCands[hit.ID]
		results[i] = common.SearchResult{Document: dc.doc.toCommon(), Score: float64(dc.score), Categories: dc.doc.Categories}
	}
...
func sharesEarlierCategory(docCats []string, earlier []common.CategoryMatch) bool {
